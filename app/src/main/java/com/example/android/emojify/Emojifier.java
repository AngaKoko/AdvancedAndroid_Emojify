...
class Emojifier {
    ...
    private static final double SMILING_PORB_THRESHOLD = .15;
    ...
    private static void whichEmoji(Face face){
        ...
        boolean smiling = face.getIsSmilingProbability() > SMILING_PORB_THRESHOLD;

        boolean lefEyeClosed = face.getIsLeftEyeOpenProbability() < EYE_OPEN_PROB_THRESHOLD;
        boolean righEyeClosed = face.getIsRightEyeOpenProbability() < EYE_OPEN_PROB_THRESHOLD;

        // TODO (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        Emoji emoji;
        if(smiling){
            if(lefEyeClosed && !righEyeClosed){
                emoji = Emoji.LEFT_WINK;
            }else if(righEyeClosed && !lefEyeClosed){
                emoji = Emoji.RIGHT_WINK;
            }else if(lefEyeClosed){
    ...
    //Enum class for all possible emoji
    private enum Emoji{
        SMILE,
        FROWN,
        LEFT_WINK,
